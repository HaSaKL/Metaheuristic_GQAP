...
// Costructor - initialize used vectors and make sure parameters have the right relation to each other
GRASP_ReactiveAlpha::GRASP_ReactiveAlpha(GQAP & _p, double _maxAlpha, double _minAlpha, int _numAlpha, int _reCalc) {
	
	// for easier calculataion calculate and save numAlpha, minAlpha and stepAlpha
	numAlpha = _numAlpha;
	minAlpha = _minAlpha;
	
	// Assertion of correct parameter value relation
...
// initializeation of the process
void GRASP_ReactiveAlpha::init(GQAP & _p) {
	...
	// Reset the average solution value for a given alpha to the current best know (random) solution
...
double GRASP_ReactiveAlpha::operator ()(GQAP& _p) {
